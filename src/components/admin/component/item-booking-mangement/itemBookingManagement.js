...
class ItemBookingManagement extends Component {
  render() {
    ...
    let classStatus = dataBooking.status === 'paid' ? 'text-warning' : 'text-danger';
    return (
      <tr>
        <td>{index + 1}</td>
        <td className="card w-75 mx-auto mt-1 text-white bg-success">{dataBooking.id.slice(0,8)}</td>
        ...
        <td className={`text-uppercase ${classStatus}`} style={{fontWeight: '600'}}>{dataBooking.status}</td>
        <td>
          <Link
